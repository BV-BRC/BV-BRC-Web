        ...
        try {
	      ...
	      console.log('processTree pIDs ', pIDs);
      	  ...
      	  if (ids[0].match(/^fig/) || ids[0].match(/CDS/)) {
			  ...
			  /*
			  if (ids[0].match(/^fig/)) {
			  	query = 'in(patric_id,(' + pIDs.join(',') + '))&select(feature_id, genome_id)&limit(25000)';
			  }
			  else {
			  	query = 'in(feature_id,(' + pIDs.join(',') + '))&select(feature_id, genome_id)&limit(25000)';
			  }

			  var url = PathJoin(this.apiServer, 'genome_feature') + '/';
			  console.log('onNodeSelection before query, query', query);
			  console.log('url', url);
          ...
          pressAndHold: function (selection, button, opts, evt) {
            ...
            popup.open({
              popup: new PerspectiveToolTipDialog({
                ...
                perspectiveUrl: '/view/GenomeList/?in(genome_id,(' + genome_ids.join(',') + '))', target: 'blank'
              }),
              around: button,
              orient: ['below']
            });
