define([
  ...
  '../PageGrid', '../formatter', '../ProteinStructureGridContainer', '../../util/PathJoin', 'dojo/request', 'dojo/_base/lang', '../../auth/authHeaders'
], function (
  declare, TabViewerBase, on, Topic,
  domClass, ContentPane, domConstruct, QueryToEnglish,
  Grid, formatter, ProteinStructureGridContainer,
  PathJoin, xhr, lang,
  authHeader) {
  return declare([TabViewerBase], {
    baseClass: 'ProteinStructureList',
    disabled: false,
    containerType: 'protein_structure',
    query: null,
    ...
    _setQueryAttr: function (query) {
      ...
      var url = PathJoin(this.apiServiceUrl, 'protein_structure', '?' + (this.query) + '&limit(1)'); // &facet((field,genome_id),(limit,35000))");
    ...
    onSetQuery: function (attr, oldVal, newVal) {
      ...
      this.queryNode.innerHTML = '<span class="queryModel">Protein Structures: </span>  ' + content;
    },

    setActivePanelState: function () {

      var active = (this.state && this.state.hashParams && this.state.hashParams.view_tab) ? this.state.hashParams.view_tab : 'structures';
    ...
    onSetTotalStructures: function (attr, oldVal, newVal) {
      ...
      this.totalCountNode.textContent = ' ( ' + newVal + '  Structures ) ';
    },
