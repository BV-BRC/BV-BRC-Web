define([
  ...
  '../PageGrid', '../formatter', '../SerologyGridContainer', '../../util/PathJoin', 'dojo/request', 'dojo/_base/lang', '../../auth/authHeaders'
], function (
  declare, TabViewerBase, on, Topic,
  domClass, ContentPane, domConstruct, QueryToEnglish,
  Grid, formatter, SerologyGridContainer,
  PathJoin, xhr, lang,
  authHeader) {
  return declare([TabViewerBase], {
    baseClass: 'SerologyList',
    disabled: false,
    containerType: 'serology_data',
    query: null,
    defaultTab: 'serology',
    perspectiveLabel: 'Serology List View',
    ...
    _setQueryAttr: function (query) {
      ...
      var url = PathJoin(this.apiServiceUrl, 'serology', '?' + (this.query) + '&limit(1)'); // &facet((field,genome_id),(limit,35000))");
    ...
    setActivePanelState: function () {
      ...
      var active = (this.state && this.state.hashParams && this.state.hashParams.view_tab) ? this.state.hashParams.view_tab : 'serology';
    ...
    postCreate: function () {
      ...
      this.watch('totalFeatures', lang.hitch(this, 'onSetTotalSerology'));
