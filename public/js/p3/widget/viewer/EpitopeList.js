define([
  ...
  '../PageGrid', '../formatter', '../EpitopeGridContainer', '../../util/PathJoin', 'dojo/request', 'dojo/_base/lang', '../../auth/authHeaders'
], function (
  declare, TabViewerBase, on, Topic,
  domClass, ContentPane, domConstruct, QueryToEnglish,
  Grid, formatter, EpitopeGridContainer,
  PathJoin, xhr, lang,
  authHeader) {
  return declare([TabViewerBase], {
    baseClass: 'EpitopeList',
    disabled: false,
    containerType: 'epitope_data',
    query: null,
    defaultTab: 'epitope',
    perspectiveLabel: 'Epitope List View',
    ...
    _setQueryAttr: function (query) {
      ...
      var url = PathJoin(this.apiServiceUrl, 'epitope', '?' + (this.query) + '&limit(1)'); // &facet((field,genome_id),(limit,35000))");
      ...
      xhr.get(url, {
        ...
        console.error('Error Retreiving Epitopes: ', err);
      });
    ...
    onSetQuery: function (attr, oldVal, newVal) {
      ...
      this.queryNode.innerHTML = '<span class="queryModel">Epitopes: </span>  ' + content;
    },

    setActivePanelState: function () {

      var active = (this.state && this.state.hashParams && this.state.hashParams.view_tab) ? this.state.hashParams.view_tab : 'epitope';
    ...
    postCreate: function () {
      ...
      this.watch('totalFeatures', lang.hitch(this, 'onSetTotalEpitope'));
    ...
    onSetTotalEpitope: function (attr, oldVal, newVal) {
      // console.log("ON SET TOTAL GENOMES: ", newVal);
      this.totalCountNode.textContent = ' ( ' + newVal + '  Epitope) ';
    },
