define([
  ...
  return declare([ViewerBase], {
    ...
    postCreate: function () {
      ...
      var hasReassortments = this.state.search.match(/hasReassortments=.+?(?=&|$)/);
      ...
      if (hasReassortments && !isNaN(hasReassortments.index)) {
        hasReassortments = hasReassortments[0].split('=')[1];
        hasReassortments = (hasReassortments === 'true') || (hasReassortments === '1');
      } else { hasReassortments = false; }
      ...
      if (folderCheck && !isNaN(folderCheck.index)) {
        ...
        WorkspaceManager.getObjects([objPath]).then(lang.hitch(this, function (objs) {
          ...
          if (typeof obj.data == 'string') {
            ...
            // If hasReassortments was included in the URL, the file was generated by TreeSort and
            // needs treeDat option customization.
            if (hasReassortments) {
               if (!treeDat.options) { treeDat.options = {}; }
               treeDat.options.branchWidthDefault = 0.1;
               treeDat.options.minBranchLengthValueToShow = 0.001;
               treeDat.options.phylogram = true;
               treeDat.options.showInternalLabels = false;
