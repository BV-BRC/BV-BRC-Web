define([
	...
	xhr,lang,Chart2D,Theme,MoveSlice,ChartTooltip,
	domConstruct
){
	return declare([WidgetBase,Templated], {
		...
		getSummaryData: function(){
			xhr.get(this.apiServiceUrl + "/genome_feature/?eq(genome_id," + this.genome.genome_id + ")&limit(1)&facet((field,feature_type),(mincount,1))&http_accept=application/solr+json", {handleAs: "json", headers:{"accept":"application/solr+json"}}).then(lang.hitch(this,function(data){
			...
			xhr.get(this.apiServiceUrl + "/sp_gene/?eq(genome_id," + this.genome.genome_id + ")&limit(1)&facet((field,property),(mincount,1))&http_accept=application/solr+json", {handleAs: "json", headers:{"accept":"application/solr+json"}}).then(lang.hitch(this,function(data){
				console.log("Data: ", data);
				var spSummary = {};
				var idx = 0;
				while(idx < data.facet_counts.facet_fields.property.length-1){
					spSummary[data.facet_counts.facet_fields.property[idx]] = data.facet_counts.facet_fields.property[idx+1]
					idx+=2;
				}
		...
		refresh: function(){
			if (this.genome && this.genome.genome_id) {
				...
				this.genomeSummary.innerHTML = "Length: ?, Chromosomes: " + this.genome.chromosomes + ", Plasmids: " + this.genome.plasmids + ", Contigs: " + this.genome.contigs;
			}else{
