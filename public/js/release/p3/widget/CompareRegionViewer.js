define("p3/widget/CompareRegionViewer", [
	...
	return declare([Evented], {
		...
		render: function(){
			...
			*/
			...
			for(var rowi = 0; rowi < data.length; rowi++){
				...
				var depth = 0;
				var beg_intergenic_id, beg_intergenic_pos;
				var intergenic = [];
		...
		add_row: function(idx, data, track_info, row_idx){
			...
			if(boc)
				row.createLine({x1: cx1, y1: -5, x2: cx1, y2: 5}).setStroke('black');
			if(eoc)
				row.createLine({x1: cx2, y1: -5, x2: cx2, y2: 5}).setStroke('black');

			// window.console.log(data.genome_id, data.contig_length);
			// window.console.log(data.beg, data.end, cx1, cx2);
			row.createLine({x1: cx1, y1: 0, x2: cx2, y2: 0}).setStroke('black');
