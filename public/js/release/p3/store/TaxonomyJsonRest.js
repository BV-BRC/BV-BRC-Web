define("p3/store/TaxonomyJsonRest", [
   	     	...
   	     	getChildren: function(parentItem,opts){
 				...
 				when(q.total, function(total){
 					if (total<1  && nextRank){
 						...
 						var rec = _self.getChildren({genomes: parentItem.genomes,taxon_id: parentItem.taxon_id,taxon_rank: nextRank}, opts);
 						when(rec.total, function(tot){
 							def.total.resolve(tot);
 						})
 						when(rec, function(res){ def.resolve(res); },function(err){ def.reject(err);});

 					}else{
 						def.total.resolve(total)
 						when(q, function(res){ def.resolve(res)}, function(err){ def.reject(err); })
 					}
 				})
